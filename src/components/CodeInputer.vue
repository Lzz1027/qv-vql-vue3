<template>
  <div class="code-inputer">
    ...
    <div class="code-runner">
      ...
      <a-button style="float: left; margin-left: 10px;" type="default" :size="size">
        <template #icon>
          <ScissorOutlined />
        </template>
      ...
      <!-- 执行查询 -->
      <a-button @click="handleQueryExecute" style="float: right;" type="primary" :size="size">
        <template #icon>
          <CaretRightOutlined />
        </template>
      ...
      <!-- 生成图 -->
      <a-button @click="handleGraphGenerate" style="float: right; margin-right: 10px;" type="primary" :size="size">
        <template #icon>
          <RedoOutlined />
        </template>
...
import { CopyOutlined, ScissorOutlined, SnippetsOutlined, DeleteOutlined, CaretRightOutlined, RedoOutlined } from '@ant-design/icons-vue'
...
const InitEditor = () => {
  ...
  editor.value = monaco.editor.create(document.getElementById('monacoContainer'), {
...
const handleGraphGenerate = async () => {
  ...
  console.log('generate graph start');
  let result
  // 修改为双引号
  let realQuery = query.value.replaceAll('\'', '\"')
  try {
    ...
    console.log(neo4jStore.vqlGraph);

  } catch (err) {
    console.log(err);
  }
  console.log('generate graph end');
}
